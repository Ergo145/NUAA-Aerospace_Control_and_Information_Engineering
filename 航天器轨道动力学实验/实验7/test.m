...
% 注意：这里的 lambert 函数直接返回了 orbit 结构体，包含了轨迹点，无需再积分
% 6.1 去程
...
% 7.2 绘制轨道 (Code A 风格：蓝色地球轨道，黑色小行星轨道)
...
% 7.3 绘制关键点 (Code A 风格标记：圆圈、三角、方块)
...
plot3(p_a3(1), p_a3(2), p_a3(3), 'kv', 'MarkerFaceColor','k', 'DisplayName', '离开小行星', 'LineWidth', 1); % 补充Code B有但Code A没写的点
plot3(p_e4(1), p_e4(2), p_e4(3), 'bs', 'MarkerFaceColor','c', 'DisplayName', '返回地球', 'LineWidth', 1);

% 7.4 绘制转移轨迹 (Code A 风格：红色去程虚线，绿色回程虚线)
% 直接使用 Code B 算出的 orbit.r，不需要 ode45 重算，速度极快
