...
for k = 1 : tf
    ...
    sqrtPrk = sqrt(PUrest);
    Xsigmark(1)=xhatUr;
    Xsigmark(2)=Xsigmark(1)+gama*sqrtPrk;
    Xsigmark(3)=Xsigmark(1)-gama*sqrtPrk;
    Xsigmarkf=zeros(1,3);
    for i=1:3
        Xsigmarkf(i)=0.5*Xsigmark(i)+25*Xsigmark(i)/(1+Xsigmark(i)^2)+8*cos(1.2*(k-1));
    end;
    %系统状态转移函数，根据具体情况编制，注意要求输入中Xak为L*（2L+1）的矩阵，uk为控制向量，输出为dimx*（2L+1）的矩阵
    xhatUr=Xsigmarkf*Wm';%均值预测
    Prkpre=Q;
    for i=1:(2*L+1),
        Prkpre=Prkpre+Wc(i)*(Xsigmarkf(i)-xhatUr)*(Xsigmarkf(i)-xhatUr);
    end;
    sqrtPrk = sqrt(PUrest);
    Xsigmarkr(1)=xhatUr;
    Xsigmarkr(2)=Xsigmarkr(1)+gama*sqrtPrk;
    Xsigmarkr(3)=Xsigmarkr(1)-gama*sqrtPrk;   
    Zsigmarkh=zeros(1,3);
    for i=1:3
        Zsigmarkh(i)=Xsigmarkr(i)^2/20;
    end;
    zrkpre=Zsigmarkh*Wm';%量测预测
    Przzk=R;
    Prxzk=0;
    for i=1:(2*L+1),
        Przzk=Przzk+Wc(i)*(Zsigmarkh(i)-zrkpre)*(Zsigmarkh(i)-zrkpre);
        Prxzk=Prxzk+Wc(i)*(Xsigmarkf(i)-xhatUr)*(Zsigmarkh(i)-zrkpre);
    end
    % 量测更新
    vrnewk=y-zrkpre;
    K=Prxzk/Przzk;
    xhatUr=xhatUr+K*vrnewk;
    PUrest=Prkpre-K*Przzk*K';
    ...
    xhatUrArr = [xhatUrArr xhatUr];
end

t = 0 : tf;

figure;
plot(t, xArr,'-',t,xhatPartArr,'o-');
...
figure;
plot(t, xArr, 'b.', t, xhatPartArr, 'ko-',t,xhatUArr,'ro-',t,xhatUrArr,'r*-',t,xhatArr,'--');
set(gca,'FontSize',12); set(gcf,'Color','White'); 
xlabel('time step'); ylabel('state');
legend('True state', 'Particle filter estimate','UKF','UKF-REDRAW','EKF'); 
