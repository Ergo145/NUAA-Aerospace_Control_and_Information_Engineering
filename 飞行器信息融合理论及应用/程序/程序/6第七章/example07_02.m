...
deltat = 1;
q3 = 0.001;q2 = 0.01;q1 = 0.1;
F = [0 1 0;0 0 1;0 0 0];
G = zeros(3,3);
G(3,3) = q3;G(1,1) = q1; G(2,2) = q2;
W = zeros(3,3);
W(3,3) = 1;W(2,2) = 1; W(1,1) = 1;
A = zeros(6,6);
A(1:3,1:3) = -1*F;
A(1:3,4:6) = G*W*G';
A(4:6,4:6) = F';
...
for j = 1:iterative_number
    ...
    randn('state', sum(100*clock));
    qk3 = sqrt(Q(3,3))*randn(span,1);
    xtrue(1,1) = qk1(span);
    xtrue(1,2) = 1.0+qk2(span);
    xtrue(1,3) = qk3(span);
    for i = 2:span
        xtrue(i,1) = xtrue(i-1,1)+deltat*xtrue(i-1,2)+deltat^2/2*xtrue(i-1,3)+qk1(i-1);
        xtrue(i,2) =                     xtrue(i-1,2)    +deltat*xtrue(i-1,3)+qk2(i-1);
        xtrue(i,3) =                                             xtrue(i-1,3)+qk3(i-1);
    end
...
%     z = z';
    ...
    xi_pre(1) = 0; xi_pre(2) = 1.0; xi_pre(3)=0.0;% initial state prediction
    Pi_pre = zeros(3);
    Pi_pre(1,1)=(xi_pre(1)-xtrue(1,1))^2;
    Pi_pre(2,2)=(xi_pre(2)-xtrue(1,2))^2;
    Pi_pre(3,3)=(xi_pre(3)-xtrue(1,3))^2;
